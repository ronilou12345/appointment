...
export default function DoctorProfileList({ doctors }: { doctors: DoctorProfile[] }) {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(300px,360px)_1fr]">
      <aside className="space-y-4 rounded-[28px] border border-border bg-background/70 p-4 shadow-sm backdrop-blur-sm">
        <div
          className="rounded-[24px] border border-white/10 p-5 text-foreground shadow-sm"
          style={{
            backgroundImage:
              "linear-gradient(to top, hsl(var(--primary) / 0.14), hsl(var(--primary) / 0.05) 45%, hsl(var(--card) / 1) 100%)",
          }}
        >
          <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-muted-foreground">Doctors</p>
          <h2 className="mt-3 text-xl font-semibold">Choose a doctor</h2>
            ...
            <button
              ...
              className={`w-full rounded-[24px] border p-4 text-left transition-all ${
                selectedDoctor.id === doctor.id
                  ...
                  : "border-border bg-card hover:border-primary/70 hover:bg-accent/20"
              }`}
              style={
                selectedDoctor.id === doctor.id
                  ? {
                      backgroundImage:
                        "linear-gradient(to top, hsl(var(--primary) / 0.12), hsl(var(--primary) / 0.04) 40%, hsl(var(--card) / 1) 100%)",
                    }
                  : undefined
              }
            >
              <div className="flex items-center gap-3">
                <Avatar className="h-14 w-14 border-2 border-white shadow-sm ring-4 ring-primary/5">
                  {doctor.avatar ? (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 truncate text-sm text-muted-foreground">{doctor.email}</p>
                </div>
              </div>
              <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                <span className="rounded-full border border-border bg-background/60 px-2 py-1">
                  {doctor.credentials.length ? doctor.credentials.join(", ") : "No credentials listed"}
                </span>
                <span className="rounded-full bg-primary/10 px-2 py-1 text-primary">{doctor.experienceLabel}</span>
              </div>
            </button>
          ))}
        </div>
      ...
      <section
        className="rounded-[30px] border border-border p-6 shadow-sm md:p-8"
        style={{
          backgroundImage:
      ...
      >
          ...
          <div className="flex items-center gap-4">
            <Avatar className="h-20 w-20 border-2 border-white shadow-md ring-4 ring-primary/5">
              {selectedDoctor.avatar ? (
                ...
                <AvatarFallback className="bg-primary/10 text-primary">{selectedDoctor.initials}</AvatarFallback>
              )}
            </Avatar>
            <div className="min-w-0">
              <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-muted-foreground">Doctor profile</p>
              <h1 className="mt-2 text-3xl font-semibold text-foreground">{selectedDoctor.name}</h1>
              <p className="mt-2 text-sm text-muted-foreground">Full doctor profile and contact details.</p>
              <p className="mt-3 text-sm font-medium text-primary">{selectedDoctor.email || "No email available"}</p>
            </div>
          </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          <div className="space-y-6 rounded-[26px] border border-border bg-background/60 p-6 shadow-sm backdrop-blur-sm">
            <div>
          ...
          <div className="space-y-6 rounded-[26px] border border-border bg-background/60 p-6 shadow-sm backdrop-blur-sm">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-border bg-background/80 p-4">
                ...
                <p className="mt-2 text-sm font-medium text-foreground">{selectedDoctor.experienceLabel}</p>
              </div>
            </div>

            <div className="rounded-2xl border border-border bg-background/80 p-4">
              <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground">Member since</p>
              <p className="mt-2 text-sm font-medium text-foreground">{selectedDoctor.joinedDate}</p>
            </div>
          </div>
        </div>
      </section>
